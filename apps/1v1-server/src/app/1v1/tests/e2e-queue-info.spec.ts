...
describe('end-to-end: case 5 - sending queue info to all players', () => {
  ...
  it('should get answer on GET_PLAYER_STATE: first player with QUEUE_INFO', (done) => {
    webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
      expect(JSON.parse(serverMessage.data as string)).toEqual({
        action: 'PLAYER_STATE',
        payload: { state: 'WAITING_FOR_QUEUE' },
      });

      webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
        expect(JSON.parse(serverMessage.data as string)).toEqual(initialQueueStateMessage);
        done();
      };
    };
  ...
  it('should get answer on JOIN_QUEUE: first player', (done) => {
    webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
      expect(JSON.parse(serverMessage.data as string)).toEqual({ action: 'JOIN_QUEUE_SUCCESS' });
      done();
    };
  ...
  it('all three players should get new queue info', (done) => {
    ...
    Promise.all([
      new Promise((resolve) => {
        webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
          expect(JSON.parse(serverMessage.data as string)).toMatchObject(queueInfoMessage);
          resolve(null);
        };
      }),
      new Promise((resolve) => {
  ...
  it('should get answer on LEAVE_QUEUE: first player', (done) => {
    webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
      expect(JSON.parse(serverMessage.data as string)).toEqual({ action: 'LEAVE_QUEUE_SUCCESS' });
      done();
    };
    ...
    Promise.all([
      new Promise((resolve) => {
        webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
          ...
          webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
            const parsedMessage: PickbanStepMessageInterface = JSON.parse(serverMessage.data as string);
  ...
  it('should send PICKBAN_STEPS to both players after first ban', (done) => {
    ...
    Promise.all([
      new Promise((resolve) => {
        webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
          expect(JSON.parse(serverMessage.data as string)).toMatchObject(
  ...
  it('should auto-ban a map if there is no response from player (3rd ban)', (done) => {
    Promise.all([
      new Promise((resolve) => {
        webSocketFirst.onmessage = (serverMessage: MessageEvent) => {
          const pickbanSteps: PickbanStepMessageInterface = JSON.parse(serverMessage.data as string);

          expect(
            pickbanSteps.payload.match.maps.filter(
