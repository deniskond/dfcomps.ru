...
export class RaceController {
  ...
  private async getPlayerInfo(playerName: string): Promise<PlayerInfo | undefined> {
    ...
    const url = process.env.NODE_ENV === 'production' ? 'https://dfcomps.ru/api/profile/search' : 'http://localhost:4001/profile/search';
    try {
  ...
  private async getUserIdAuth(token: string | undefined): Promise<number | undefined> {
    ...
    // return undefined;
    const url = process.env.NODE_ENV === 'production' ? 'https://dfcomps.ru/api/auth/user' : 'http://localhost:4001/auth/user';
    console.log(url);
    console.log(token);
    try {
      const response = await axios.get(url, {
