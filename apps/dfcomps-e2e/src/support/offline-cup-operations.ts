...
export function addOfflineCup(mapName: string, options: { needToLogin: boolean } = { needToLogin: true }) {
  ...
  cy.intercept(
    {
      method: 'GET',
      url: `/admin/cups/get-worldspawn-map-info?map=${mapName}`,
    },
    {
      statusCode: 200,
      body: {
  ...
  // filling the form
  cy.get('[data-test-id=offline-cup-fullname-input]').type(faker.lorem.words());
